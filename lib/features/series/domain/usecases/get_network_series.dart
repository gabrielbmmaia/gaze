import 'package:gaze/core/enums/networks.dart';
import 'package:gaze/core/usecases/usecase.dart';
import 'package:gaze/core/utils/typedefs.dart';
import 'package:gaze/features/series/domain/models/series_model.dart';
import 'package:gaze/features/series/domain/repositories/series_repo.dart';

class GetNetworkSeriesUseCase
    extends UseCaseWithParams<List<SeriesModel>, Networks> {
  const GetNetworkSeriesUseCase(this._repo);

  final SeriesRepo _repo;

  @override
  ResultFuture<List<SeriesModel>> call(Networks params) =>
      _repo.getNetworkSeries(params);
}
